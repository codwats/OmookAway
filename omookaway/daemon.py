...
class StateFiles:
    ...
    def load(self, now: float) -> Engine:
        try:
            return Engine.restore(json.loads(self.state_path.read_text()), now)
        except (FileNotFoundError, KeyError, TypeError, ValueError, json.JSONDecodeError):
            return Engine(Config(), now)

    def publish(self, engine: Engine, now: float) -> None:
        self._write(self.state_path, engine.snapshot(now), private=True)
        self._write(self.status_path, engine.status(now), private=False)
...
class Daemon:
    def __init__(self, socket_path: Path, files: StateFiles) -> None:
        ...
        self.files = files
        self.engine = files.load(time.monotonic())
    ...
    async def handle(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        try:
            ...
            if request.get("type") == "status":
                response = self.engine.status(now)
            else:
                response = self.engine.apply(request, now)
                self.files.publish(self.engine, now)
    ...
    async def tick(self) -> None:
        while True:
            ...
            now = time.monotonic()
            self.engine.apply({"type": "time"}, now)
            self.files.publish(self.engine, now)

    async def run(self) -> None:
        self.socket_path.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
        self.socket_path.unlink(missing_ok=True)
        self.files.publish(self.engine, time.monotonic())
